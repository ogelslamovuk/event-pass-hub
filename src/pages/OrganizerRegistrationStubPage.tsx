    ...
    <div className="min-h-screen px-4 py-8" style={{ background: "#0B0F14", color: "#F5F7FA" }}>
      ...
      <div className="mx-auto max-w-4xl rounded-2xl border p-6 space-y-6" style={{ borderColor: "rgba(255,255,255,0.10)", background: "#111A24" }}>
        ...
        <section className="space-y-3">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="flex items-center gap-2">
              <input className="h-10 rounded px-3 bg-[#0F1620] border flex-1" placeholder="Полное наименование" value={form.legalName} onChange={(e) => setForm((p) => ({ ...p, legalName: e.target.value }))} />
              <FieldHelp text="Официальное наименование юридического лица." />
            </div>
            <div className="flex items-center gap-2">
              <input className="h-10 rounded px-3 bg-[#0F1620] border flex-1" placeholder="Регистрационный номер" value={form.registrationNumber} onChange={(e) => setForm((p) => ({ ...p, registrationNumber: e.target.value }))} />
              <FieldHelp text="Номер госрегистрации или УНП организации." />
            </div>
            ...
            <div className="flex items-center gap-2">
              <input className="h-10 rounded px-3 bg-[#0F1620] border flex-1" placeholder="Контактный телефон" value={form.contactPhone} onChange={(e) => setForm((p) => ({ ...p, contactPhone: e.target.value }))} />
              <FieldHelp text="Телефон для связи по заявке и уточнений." />
            </div>
            <div className="flex items-center gap-2">
              <input className="h-10 rounded px-3 bg-[#0F1620] border flex-1" placeholder="Email" value={form.email} onChange={(e) => setForm((p) => ({ ...p, email: e.target.value }))} />
              <FieldHelp text="Email для уведомлений о решении и доработках." />
            </div>
        ...
        <section className="space-y-3">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="flex items-center gap-2">
              <input className="h-10 rounded px-3 bg-[#0F1620] border flex-1" placeholder="ФИО" value={form.director.fullName} onChange={(e) => updateDirector({ fullName: e.target.value })} />
              <FieldHelp text="ФИО руководителя в точном соответствии с документом." />
            </div>
        ...
        <section className="space-y-2">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div className="flex items-center gap-2">
              <input className="h-10 rounded px-3 bg-[#0F1620] border flex-1" placeholder="Желаемый логин" value={form.accountCredentials.login} onChange={(e) => setForm((p) => ({ ...p, accountCredentials: { ...p.accountCredentials, login: e.target.value } }))} />
              <FieldHelp text="Логин для входа в кабинет организатора." />
            </div>
            <div className="flex items-center gap-2">
              <input className="h-10 rounded px-3 bg-[#0F1620] border flex-1" type="password" placeholder="Пароль" value={form.accountCredentials.password} onChange={(e) => setForm((p) => ({ ...p, accountCredentials: { ...p.accountCredentials, password: e.target.value } }))} />
              <FieldHelp text="Пароль для входа, не короче 6 символов." />
            </div>
          </div>
        </section>
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          <div className="flex items-center gap-2">
            ...
            <FieldHelp text="Сохраняет текущую форму без отправки на рассмотрение." />
          </div>
          <div className="flex items-center gap-2">
            ...
            <FieldHelp text="Отправляет текущую версию как новую попытку." />
          </div>
